...
class DeviceProfile:
    ...
    def from_dict(cls, data: dict) -> "DeviceProfile":
        ...
        for it_key, it_val in data.get("interface_types", {}).items():
            # Xử lý control mapping
            control = {
                k: SemanticData(**v) for k, v in it_val.get("control", {}).items()
            }
            # Xử lý controlled mapping
            controlled = {
                k: SemanticData(**v) for k, v in it_val.get("controlled", {}).items()
            }
